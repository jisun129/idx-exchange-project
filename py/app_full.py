"""
Full-feature Prediction App (Streamlit)

Takes all 28 features used by the main pipeline's tuned XGBoost model (see
02_preprocessing.ipynb Week 6 / 06_evaluation.ipynb) as input and outputs a predicted
sale price.

Requires the files produced by train_full_model.py: full_price_model.pkl,
feature_defaults.json, school_district_options.json, levels_options.json.
...
MODEL_PATH = "full_price_model.pkl"
DEFAULTS_PATH = "feature_defaults.json"
DISTRICTS_PATH = "school_district_options.json"
LEVELS_PATH = "levels_options.json"
...
st.title("\U0001F3E1 California Home Price Predictor")
st.write(
...
try:
    ...
    st.error(
        f"Missing file: `{e.filename}`. Run `python train_full_model.py` first to "
        "generate the model and its lookup files (see the project README)."
    )
    st.stop()
...
if st.button("Predict Price", type="primary"):
    ...
    st.caption(
        "Predicted with the full 28-feature XGBoost pipeline (same hyperparameters as "
        "06_evaluation.ipynb's final model). `BedBathRatio` and `PropertyAge` were computed "
        "automatically from your Beds/Baths/Year Built inputs above."
    )
